...
class TestPollQueuesResilience:
    ...
    def _build_app(self, app_module, tmp_path, monkeypatch):
        monkeypatch.setattr(app_module, "SETTINGS_FILE", tmp_path / "settings.json")
        app = app_module.App()
        app.withdraw()
        return app

    def test_bad_thumbnail_does_not_break_polling(self, app_module, tmp_path, monkeypatch):
        app = self._build_app(app_module, tmp_path, monkeypatch)
        try:
    ...
    def test_good_thumbnail_still_renders_after_a_bad_one(self, app_module, tmp_path, monkeypatch):
        app = self._build_app(app_module, tmp_path, monkeypatch)
        try:
    ...
    def test_poll_queues_reschedules_even_if_inner_step_raises(self, app_module, tmp_path, monkeypatch):
        app = self._build_app(app_module, tmp_path, monkeypatch)
        try:
